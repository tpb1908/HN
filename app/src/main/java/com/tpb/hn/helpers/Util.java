...
public class Util {
    ...
    public static int getApproximateNumberOfItems(FeedActivity.Section section) {
        switch(section) {
            case TOP:
                return 500;
            case BEST:
    ...
    public static FeedActivity.Section getSection(Context context, String section) {
        final String[] sections = context.getResources().getStringArray(R.array.nav_spinner_items);
        if(section.equals(sections[0])) return FeedActivity.Section.TOP;
        if(section.equals(sections[1])) return FeedActivity.Section.NEW;
        if(section.equals(sections[2])) return FeedActivity.Section.BEST;
        if(section.equals(sections[3])) return FeedActivity.Section.ASK;
        if(section.equals(sections[4])) return FeedActivity.Section.SHOW;
        if(section.equals(sections[5])) return FeedActivity.Section.JOB;
        return FeedActivity.Section.SAVED;
    }
